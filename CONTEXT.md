...
Generated from actual folder structure on 2026-06-27.

## Route Map

| URL                        | File                                                   | Type              | Auth required |
|----------------------------|--------------------------------------------------------|-------------------|---------------|
| `/`                        | `src/app/page.tsx`                                     | Server Component  | No (landing page with pricing section; redirects to `/dashboard` if authed; forwards `?code=` to `/auth/callback`) |
| `/login`                   | `src/app/(auth)/login/page.tsx`                        | Client Component  | No |
| `/signup`                  | `src/app/(auth)/signup/page.tsx`                       | Client Component  | No (accepts `?invite=` and `?plan=` params) |
...
| `/setup`                   | `src/app/(dashboard)/setup/page.tsx`                   | Client Component  | Yes (accepts `?plan=`; redirects to Stripe checkout if plan present) |
...
| `/terms`                   | `src/app/(legal)/terms/page.tsx`                       | Server Component  | No |
| `/privacy`                 | `src/app/(legal)/privacy/page.tsx`                     | Server Component  | No |
...
| `(legal)`     | `src/app/(legal)/layout.tsx`            | White bg, logo header, footer with links to `/terms` and `/privacy` |
...
| `src/app/(dashboard)/dashboard/actions.ts`      | `regenerateInviteCodeAction`, `dismissNotificationAction`, `acknowledgeLeaveAction`, `forceLeaveAction` |
...
src/
  app/
    (auth)/
      ...
      login/page.tsx
      signup/
        page.tsx                    ← accepts ?invite= and ?plan=; Enter key submits
        confirm/page.tsx
    ...
    (dashboard)/
      ...
      dashboard/
        page.tsx                    ← stat cards (clickable), HouseholdSummaryCard (owner only), NotificationBanner, premium badge, invite section
        actions.ts                  ← regenerateInviteCodeAction, dismissNotificationAction, acknowledgeLeaveAction, forceLeaveAction
      setup/page.tsx                ← household creation form; accepts ?plan=; redirects to Stripe checkout if plan present
      chores/
        page.tsx
        actions.ts
      bills/
    ...
    (legal)/
      layout.tsx                    ← white bg, logo header, footer with /terms and /privacy links
      terms/page.tsx                ← Terms of Service (iDaMar Labs LLC, Ohio LLC)
    ...
    layout.tsx                      ← root layout, metadata, Google Translate scripts
    page.tsx                        ← landing page with hero, features, pricing section, footer; LanguageSwitcher in nav
    globals.css
  components/
    ...
    household/
      ...
      DepartureRequestBanner.tsx    ← client component; shows pending departure banner; owner can force-complete
      ...
      HouseholdSummaryCard.tsx      ← client component; per-member bills/chores/rules summary table (owner only)
    ...
    LanguageSwitcher.tsx            ← client component; Google Translate cookie-based language picker (EN/ES/FR/中文)
  lib/
  ...
  services/
    ...
    household.service.ts            ← getUserHousehold, getHouseholdMembers, createHousehold, joinHousehold, requestLeave, cancelLeave, acknowledgeLeave, forceCompleteLeave, renameHousehold, regenerateInviteCode, getPendingDepartureRequest
  ...
  types/index.ts                    ← shared TypeScript interfaces (Household includes invite_code, stripe_customer_id, stripe_subscription_id; BillShare includes payment_note; Bill includes recurring)
...
/setup → POST /api/households → createHousehold service
  ...
  → if ?plan=monthly or ?plan=yearly: POST /api/checkout → redirect to Stripe Checkout
